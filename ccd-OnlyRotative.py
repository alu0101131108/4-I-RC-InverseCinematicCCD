...
while (dist > EPSILON and abs(prev-dist) > EPSILON/100.):
  ...
  for i in range(len(th)):
    ...
    xLast = O[i][lastIndex][0]
    yLast = O[i][lastIndex][1]
    xTarget = O[i][targetIndex][0]
    yTarget = O[i][targetIndex][1]
    
    xToLast = xLast - xTarget
    yToLast = yLast - yTarget

    xToObjective = objetivo[0] - xTarget
    yToObjective = objetivo[1] - yTarget

    alfaToLast = atan2(yToLast, xToLast)
    alfaToObjective = atan2(yToObjective, xToObjective)
    alfa = alfaToObjective - alfaToLast
    
    th[targetIndex] = th[targetIndex] + alfa
